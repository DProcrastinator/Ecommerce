    ...
    public function edit()
    {
        if(Request::has('post')){
            ...
            if(empty($file->product_image_path->name)){
                       ...
                       $file_error['image_path'] =['image is invalid, Please try again'];
            }
            if(empty($file->hover_image_path->name)){
                $file_error['hover_image_path'] = ['The product hover image is required'];
            }
            else if(!UploadFile::isImage($hoverimagefilename)){
                     $file_error['hover_image_path'] =['image is invalid, Please try again'];
                }
            ...
            if($validate -> hasError()){
              ...
              if(!empty($file_error)){
                dd($file_error);
               return view('admin/products/product',['errors'=> $file_error]); 
           }
